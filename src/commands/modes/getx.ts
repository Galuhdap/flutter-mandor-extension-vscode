...
// =========================
// IMPLEMENTASI MODE GETX
// =========================
export async function initStructureGetX() {
  ...
  if (!fs.existsSync(pubspecPath)) {
    vscode.window.showErrorMessage(
      "Flutter Mandor: Not a Flutter project"
    );
    return;
  }
  ...
  try {
    ...
    // =========================
    // Inject Android Flavor
    // =========================
    await injectAndroidFlavor(rootPath);

    vscode.window.showInformationMessage(
      "Flutter Mandor: Mode Standard generated completely!"
    );
  } catch (error: any) {
    vscode.window.showErrorMessage(
      `Flutter Mandor error: ${error.message}`
    );
  }
}
